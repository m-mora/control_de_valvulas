...
#define CLEARED       0
#define PRESSED       1
#define RESET_PRESS_TIME  3000 // 3 seconds 
...
// 
int lastState = HIGH;                // button was pressed set it low
...
// if button is pressed for 3 seconds need to reset wifi credentials
bool check_reset()
{
  currentState = digitalRead(RESET_BUTTON);
  if (lastState == HIGH && currentState == LOW) 
  {
    pressedTime = millis();
    Serial.printf ("pressed at %ld\n",pressedTime);
  } else if (lastState == LOW && currentState == HIGH)
  {
  // button released
    releasedTime = millis();
    Serial.printf("Released at %ld\n",releasedTime);
    if ((releasedTime - pressedTime) > RESET_PRESS_TIME)
    {
      return true;
    } 
  }
